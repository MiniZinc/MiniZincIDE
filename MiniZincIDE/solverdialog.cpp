...
void SolverDialog::on_solvers_combo_currentIndexChanged(int index)
{
    if (index<solvers.size()) {
        ...
        ui->supports_replay->setChecked(solvers[index].supports_replaying);
        ...
        ui->supports_replay->setChecked(false);
        ui->mznpath->setText("");
        ui->backend->setText("");
...
void SolverDialog::on_updateButton_clicked()
{
    ...
    solvers[index].supports_replaying = ui->supports_replay->isChecked();
